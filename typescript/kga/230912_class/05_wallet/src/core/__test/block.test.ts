...
// describe : 테스트 코드의 그룹 단위
    ...
    */
    ...
    const ec = new elliptic.ec("secp256k1");
        /* 타원 곡선 알고리즘이 뭐고, 어디에 쓰지?
            - 의의
                - 타원 곡선 암호화는 '공개키 암호화' 에서 널리 사용됨. 
                - 비트코인, 이더리움은 '타운 곡선 알고리즘' 을 사용함. 

            - 목적(어디에 쓰지?)
                - 키 생성 및 디지털 서명!(서명 쓰는 이유는, 진짜 네가 한게 맞는지 확인하기 위해서⭐⭐)
            ...
            - '타원 곡선의 기준점(base point)' 이란? 
                - 전달하는 사람과 받는 사람 등 모든 사람들은, 공통적으로, 타운곡선의 한 점을 알고 있어야 함 ⭐⭐⭐
                - 발신사, 수신자가 모두 알고 있는 점을 '타원 곡선의 기준점(base point)' 이라고 함.
                - '기준점' 에 따라, '별명' 이 붙는다.

            - '기준점'은 '어떤 곡선' 상에 있고, 구체적으로 어떤 식이 나오는거지? 
                y^2 = x^3 + ax + b
                    이 방정식에 만족하는 곡선이 그어짐
            ...
            - '개인키' 가 정해지는 과정 
                1. DJ 가 트랜잭션을 만듦 ex) 물건을 샀다. 
                2. 볼펜으로, '타원 곡선의 지정 범위 내' 에서 '개인키' 의 값을 정함
                    - 즉, (1 ~ n-1) 까지의 정수 범위. (범위 내의 정수)
                        ...
                        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141 에서 -1 을 하면, 개인키가 들어갈 수 있는 범위가 된다. 
                    - 만약, 이 범위를 넘어가면, 개인키 에러가 난다. 
                    - 그리고 개인키는 '⭐⭐임의로, 랜덤으로 지정⭐⭐' 된다. 
                    
            - 이렇게 만든 개인키로 '서명'을 만든다면? 그러면, '서명의 구성요소' 는? 
                - 전자 서명을 만들 때 2개의 서명이 필요함. 서명 r 과 s 
...
/*  이제 이 밑에는 테스트를 해봐
    채굴 - 보상 - 첫 번째 블록에 추가 > 잔액이 지갑에 있고 
    다른 지갑에 있고 > pool 에 있고 > utxo 에 미사용 객체 있는 거 
*/